...
import { registerFeatureVerifyCommand } from './commands/feature-verify.js';
...
export function createCli(): Command {
  ...
  program
    ...
    .action(async (options) => {
      const { getLatestRunResult, getRunResult, generateHtmlReport, loadConfig } = await import('@qualyx/core');
  ...
  program
    ...
    .option('-f, --format <format>', 'Output format: table or json', 'table')
    .action(async (options) => {
      const { getRecentRunResults, getFailedRunResults } = await import('@qualyx/core');
      const chalk = (await import('chalk')).default;
  ...
  registerFeatureVerifyCommand(program);
